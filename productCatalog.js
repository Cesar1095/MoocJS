...
function processTypeSearch(searchId){
...
function processPriceSearch(searchId){
  	return function(difference) {
	    api.searchProductsByPrice(searchId, difference).then(function(val){
  	      updateTable('similarTable',val);
    	}).catch(function(val){
      	  alert(val);
	    });
...
document.getElementById("priceInputButton").addEventListener('click',function(){
    processPriceSearch(document.getElementById('priceInput').value)(50);
});
